...
for (region, approx) in zip(eligible_regions, approxs):
    cv2.drawContours(out, [approx], 0, (int(np.random.uniform(256)), int(np.random.uniform(256)), int(np.random.uniform(256))), 2)
...
for (region, approx) in zip(eligible_regions, approxs):
    ...
    warp = cv2.warpPerspective(img, transform, OUTPUT_DIM)
